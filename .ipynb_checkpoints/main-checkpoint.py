...
def summarize_content(transcript: str, client: OpenAI, model: str = "mistralai/mistral-7b-instruct:free") -> str:
    try:
        response = client.chat.completions.create(
            extra_headers={
                "HTTP-Referer": "your_site_url_here",  # Buraya kendi sitenizin URL'sini ekleyin
                "X-Title": "Your App Name Here"  # Buraya uygulamanızın adını ekleyin
            },
            model=model,
            messages=[
...
def tfidf_analysis(text: str, max_words=50, min_score=0.080):
    ...
    # Skoru min_score'tan büyük olan kelimeleri filtrele
    filtered_keywords = {feature_names[i]: scores[i] for i in range(len(scores)) if scores[i] >= min_score}
    
    # Kelimeleri skorlarına göre sırala ve max_words kadarını al
...
def lda_analysis(text, num_topics=50, passes=15):
    ...
    # Konuları ve anahtar kelimeleri çıkar
    topics = []
...
async def process_file(file: UploadFile = File(...)):
    # Sabit API key değeri
    api_key = "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"
    ...
    try:
        ...
        if file_extension == "pdf":
            ...
            urls = []  # Excel dosyalarında URL çıkarma işlemi yapılmıyor
        elif file_extension == "docx":
            content = extract_text_from_word(temp_file_path)
            urls = []  # Word dosyalarında URL çıkarma işlemi yapılmıyor

        # Her link için işlem yap (PDF için)
        summaries = []
        tfidf_images = []
        lda_images = []
        ...
        if file_extension == "pdf":
            ...
            # Excel veya Word dosyaları için direkt içerik analizi
            summary = summarize_content(content, get_openai_client(api_key))
            summaries.append(summary)
